...
**A note on terminology and scoring.** Our scoring substrate is MBPP+ [9], EvalPlus's augmented variant of the MBPP coding benchmark. Each MBPP+ task ships with (a) a short natural-language prompt, (b) a handful of *base* tests carried over from the original MBPP problem, and (c) many *`plus_input` cases* — typically ~100 per task — that EvalPlus automatically derives by mutating and fuzzing the canonical reference solution; they are not hand-written by us. The `plus_input` cases probe edge behavior (empty inputs, punctuation, duplicates, exact tie-breaks, off-by-one) that the natural-language prompt does not pin down. When we say **"hidden semantics"** we mean exactly that gap — the behavior the `plus_input` cases reward that the prompt leaves underspecified. A reported score of, e.g., *179/531* counts **individual `plus_input` case passes**, summed across every task in the slice; the denominator is the total number of `plus_input` cases across the slice's five tasks, not the task count. Per-task breakdowns appear in §4. We work with three slices: the **original hard slice** (5 tasks, 531 `plus_input` cases) is also the coevolution training pool; **`hard-test`** (5 tasks, 554 cases) is strictly held out and never seen during coevolution; the **fresh-failures slice** is a third pool of nine MBPP+ tasks freshly sampled from an `mbpp-plus` 60-task probe where direct generation passes the base tests but fails at least one `plus_input` case — i.e., currently-observed direct failures, separate from the hand-curated hard slices.

Our main contributions are:

1. **A runnable spec-guided synthesis pipeline.** Generator, distinguisher, user-proxy, coder, scorer, and per-episode telemetry, that increases code quality on every slice we measured. The reward function decomposes into generator and distinguisher terms with anti-regression and excess-clarification penalties (§3.4).
2. **Three MBPP+ benchmark slices for underspecified semantics.** An original hard slice (low partial direct, 101/531; doubles as the coevolution training pool), a held-out `hard-test` split (higher partial direct, 406/554; never seen during coevolution), and a nine-task fresh-failures slice (788/994). All three are selected from MBPP+ `plus_input` for hidden-edge semantics, not algorithmic depth. The hand-written pipeline raises pass rate from 19.0% to 33.7% on the original hard slice (a 77.4% relative increase) and from 73.3% to 96.2% on the held-out `hard-test` slice.
3. **A diagnostic ceiling.** A specifically crafted prompt that encodes the five hidden semantics of the original hard slice reaches 531/531, proving the architecture itself can exploit correct specs end-to-end. The remaining bottleneck is automatic spec discovery, not pipeline implementation.
